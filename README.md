...
<p>
  <a href="https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765/artifacts/9350537441"><strong>下载 2.0.7 Windows x64 安装程序</strong></a>
  ·
  <a href="https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765">查看 2.0.7 构建记录</a>
  ·
  <a href="docs/user-guide.md">阅读用户指南</a>
</p>

<p>
  <img src="https://img.shields.io/badge/version-2.0.7-13227A?style=flat-square" alt="Version 2.0.7">
...
下面的产品图来自仓库中的 [`assets/readme/ai-legal-advisor-preview.html`](assets/readme/ai-legal-advisor-preview.html)。它使用 HTML/CSS 模拟应用运行时，再由浏览器渲染截取，展示 AI法律顾问 2.0.7 的法律工作台、合同审查和合规中心。左侧底部的 **DeepSeek API 余额** 卡片即为本版本预置的 DSH-money-view 能力入口。
...
2.0.7 在 2.0.6 的 Windows x64 开箱即用体验基础上，继续保留预置的 DeepSeek API 余额卡片和上一版本的稳定性修复，并新增合同事实账本、文件读取失败阻断、真实条款引用门禁、SaaS/DPA 专项路由和固定回归测试。双击安装包后按向导完成安装，桌面会生成“AI法律顾问”快捷方式；再次双击快捷方式，应用会显示启动状态并自动打开浏览器工作台。普通用户不需要额外安装 Node.js、Python、Git 或其他开发环境。请从下方 **2.0.7 CI artifact** 下载当前版本；下载后先解压 artifact，再运行其中的 `AI法律顾问-2.0.7-x64-Setup.exe`。如果电脑上仍运行旧版 AI法律顾问，请先退出旧程序，必要时卸载旧版后再安装，以避免旧进程或文件锁阻止 NSIS 覆盖文件。

### 下载与校验

| 项目 | 信息 |
| --- | --- |
| 当前版本 Windows x64 artifact | [下载 AI法律顾问 2.0.7 Windows installer](https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765/artifacts/9350537441) |
| 安装器文件名 | `AI法律顾问-2.0.7-x64-Setup.exe` |
| 构建记录 | [GitHub Actions CI #32210040765](https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765) |
| 对应提交 | [`3ec91bb7eb`](https://github.com/tyche66/AI-Legal-Advisor/commit/3ec91bb7eb32fb442f57924c66f4437b5749de85) |
...
下载并解压 artifact 后，可以在 Windows PowerShell 中使用下面的命令校验 2.0.7 安装文件：

```powershell
Get-FileHash .\AI法律顾问-2.0.7-x64-Setup.exe -Algorithm SHA256
```
...
2.0.7 延续 2.0.6 面向实际企业法律工作的强化基础，并把 [DSH-money-view](https://github.com/tyche66/DSH-money-view) 预置为 AI法律顾问的内置余额能力。用户不需要额外安装插件；当 Harness 中最近发生过 DeepSeek 调用时，工作台会在侧栏底部展示 CNY/USD 余额、当日基准、剩余进度和最近查询时间。
...
| [DSH-money-view](https://github.com/tyche66/DSH-money-view) | 提供 DeepSeek API 余额查询、活动窗口、每日基准和 sidebar footer 卡片；2.0.7 继续保留适配版并随 Desktop 预置。 | MIT License，见 [`dsh-plugin-desktop/bundled/dsh-money-view/`](dsh-plugin-desktop/bundled/dsh-money-view/) 的 NOTICE、LICENSE 与源码快照。 |
...
[下载 AI法律顾问 2.0.7](https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765/artifacts/9350537441) · [查看源代码](https://github.com/tyche66/AI-Legal-Advisor)
...
[6]: https://github.com/tyche66/AI-Legal-Advisor/actions/runs/32210040765 "AI法律顾问 2.0.7 Windows 构建记录"
