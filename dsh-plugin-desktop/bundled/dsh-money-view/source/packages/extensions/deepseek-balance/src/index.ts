...
import type {
  ConnectionRpcEndpointMatcher, ConnectionRpcHandler, HostConnectionHandle,
...
export interface Config {
  /** DeepSeek-compatible API origin. */
  readonly baseURL?: string
  /** Credential reference used by the existing DeepSeek provider. */
  readonly apiKeyEnv?: string
  /** Fallback interval when the API response has no max-age cache hint. */
  readonly defaultRefreshIntervalMs?: number
  /** A balance refresh is useful only after a recent DeepSeek request. */
...
export function apply(ctx: Context, rawConfig: Config = {}): void {
  const config = {
    ...
    defaultRefreshIntervalMs: Math.max(MIN_REFRESH_INTERVAL_MS, Math.min(
      MAX_REFRESH_INTERVAL_MS,
      rawConfig.defaultRefreshIntervalMs ?? DEFAULT_REFRESH_INTERVAL_MS,
    )),
  ...
  const refresh = async (now: number): Promise<BalanceSnapshot> => {
    ...
    try {
      ...
      fetchedAt = now
      lastError = undefined
      const interval = cacheMaxAge(response) ?? config.defaultRefreshIntervalMs
      nextQueryAt = now + interval
      return snapshot(now)
    } catch (error: unknown) {
