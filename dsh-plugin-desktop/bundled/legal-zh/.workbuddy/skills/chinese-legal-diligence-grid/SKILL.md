...
description: "尽调问题表格工作流：并购尽调材料读取、问题抽取、尽调表格、披露问题清单、法律尽调多文档归一化。claude-for-legal-ZH/managed-agent-cookbooks/diligence-grid 的 WorkBuddy 路由技能。"
---

# 尽调问题表格工作流 — claude-for-legal-ZH（WorkBuddy 适配）

本技能把原托管 Agent cookbook 转写为 WorkBuddy 工作流。

## 源文件

- Cookbook 根目录：`managed-agent-cookbooks/diligence-grid`
- 先读：`managed-agent-cookbooks/diligence-grid/README.md`
- Agent 蓝图：`managed-agent-cookbooks/diligence-grid/agent.yaml`
- 引导示例：`managed-agent-cookbooks/diligence-grid/steering-examples.json`
- 子 Agent 蓝图：`managed-agent-cookbooks/diligence-grid/subagents`
