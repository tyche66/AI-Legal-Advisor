...
description: "法律诊所领域法律工作：法律诊所、学生案件接待、诊所备忘录、研究路线、结案移交、指导老师审阅、当事人沟通。claude-for-legal-ZH/legal-clinic 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 法律诊所 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/legal-clinic` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`legal-clinic`
- 领域画像与共享规则：`legal-clinic/CLAUDE.md`
- 原始技能目录：`legal-clinic/skills`
- 原插件说明：配置法律诊所、导入学生、运行结构化接待（intake）、以执业风险意识追踪截止日期，并在学期结束时移交案件 — 依据中国法学院法律诊所实践规范与《律师执业管理办法》构建。
...
1. 处理实质法律工作前，先读取 `legal-clinic/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/legal-clinic/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/legal-clinic:some-command` 应解释为：读取 `legal-clinic/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
