...
description: "AI 治理领域法律工作：AI 应用登记、算法安全评估、科技伦理审查、生成式 AI 合规、AI 供应商审查、AI 治理。claude-for-legal-ZH/ai-governance-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# AI 治理 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/ai-governance-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`ai-governance-legal`
- 领域画像与共享规则：`ai-governance-legal/CLAUDE.md`
- 原始技能目录：`ai-governance-legal/skills`
- 原插件说明：AI 治理实务：对拟议的 AI 应用场景进行分类登记、依据适用监管体系开展算法安全评估与科技伦理审查、审查 AI 供应商条款中的训练数据使用和责任条款、保持 AI 使用政策与实践同步。
...
1. 处理实质法律工作前，先读取 `ai-governance-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/ai-governance-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/ai-governance-legal:some-command` 应解释为：读取 `ai-governance-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
