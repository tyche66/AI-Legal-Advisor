...
description: "监管合规领域法律工作：法规动态监控、监管简报、政策差异比对、合规差距、征求意见稿、监管政策重写、行政监管。claude-for-legal-ZH/regulatory-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 监管合规 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/regulatory-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`regulatory-legal`
- 领域画像与共享规则：`regulatory-legal/CLAUDE.md`
- 原始技能目录：`regulatory-legal/skills`
- 原插件说明：监管追踪实务：监测监管法规动态，对比新规与政策库的差异，跟踪征求意见期和合规差距，编制团队周一晨会监管简报。
...
1. 处理实质法律工作前，先读取 `regulatory-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/regulatory-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/regulatory-legal:some-command` 应解释为：读取 `regulatory-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
