...
description: "税务合规与筹划领域法律工作：主体税负分流、增值税法、发票合规、金税四期、三流一致、虚开逃税红线、合法节税、税收优惠、小微与研发加计、核定征收、社保入税、税务筹划红线、税务法。claude-for-legal-ZH/tax-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 税务合规与筹划 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/tax-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`tax-legal`
- 领域画像与共享规则：`tax-legal/CLAUDE.md`
- 原始技能目录：`tax-legal/skills`
- 原插件说明：面向老板与创业者的税务合规工作流：主体税负分流、发票与金税四期合规审查、合法节税与逃税红线审查、税收优惠与政策红利梳理。所有税率、起征点、优惠额度均须运行时检索官方公告后引用，绝不硬编码。每项输出均为供律师/注册税务师复核的草稿，而非税务意见。
...
1. 处理实质法律工作前，先读取 `tax-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/tax-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/tax-legal:some-command` 应解释为：读取 `tax-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
