...
description: "法学学习与法考领域法律工作：法考、案例摘要、IRAC、课堂提问、法学写作、记忆卡片、学习计划、主观题和客观题训练。claude-for-legal-ZH/law-student 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 法学学习与法考 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/law-student` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`law-student`
- 领域画像与共享规则：`law-student/CLAUDE.md`
- 原始技能目录：`law-student/skills`
- 原插件说明：互动式案例教学训练、案例摘要（case brief）、知识体系搭建（outline builder）、法考备考（客观题+主观题）、IRAC 写作评估、学习计划制定 — 始终引导思考，不替答不代写。适配中国法学教育与国家统一法律职业资格考试。
...
1. 处理实质法律工作前，先读取 `law-student/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/law-student/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/law-student:some-command` 应解释为：读取 `law-student/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
