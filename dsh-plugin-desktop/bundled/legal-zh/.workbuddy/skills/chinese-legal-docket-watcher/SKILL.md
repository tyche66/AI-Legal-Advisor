...
description: "案件期限监控工作流：诉讼案件台账、期限监控、案件 docket、开庭和举证期限、诉讼日程提醒。claude-for-legal-ZH/managed-agent-cookbooks/docket-watcher 的 WorkBuddy 路由技能。"
---

# 案件期限监控工作流 — claude-for-legal-ZH（WorkBuddy 适配）

本技能把原托管 Agent cookbook 转写为 WorkBuddy 工作流。

## 源文件

- Cookbook 根目录：`managed-agent-cookbooks/docket-watcher`
- 先读：`managed-agent-cookbooks/docket-watcher/README.md`
- Agent 蓝图：`managed-agent-cookbooks/docket-watcher/agent.yaml`
- 引导示例：`managed-agent-cookbooks/docket-watcher/steering-examples.json`
- 子 Agent 蓝图：`managed-agent-cookbooks/docket-watcher/subagents`
