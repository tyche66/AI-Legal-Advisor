...
description: "公司与并购领域法律工作：并购尽调、重大合同披露、董事会/股东会决议、交割清单、公司合规、投后整合、公司法。claude-for-legal-ZH/corporate-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 公司与并购 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/corporate-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`corporate-legal`
- 领域画像与共享规则：`corporate-legal/CLAUDE.md`
- 原始技能目录：`corporate-legal/skills`
- 原插件说明：规模化开展并购尽调（附引用来源的表格化审查），制作披露函与交割检查表，按公司格式起草董事会决议与股东会决议，跨法域追踪主体合规期限。
...
1. 处理实质法律工作前，先读取 `corporate-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/corporate-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/corporate-legal:some-command` 应解释为：读取 `corporate-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
