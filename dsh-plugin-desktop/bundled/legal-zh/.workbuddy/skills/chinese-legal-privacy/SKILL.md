...
description: "数据合规与隐私领域法律工作：个人信息保护影响评估、PIPL、数据处理协议、DSAR、隐私政策、数据合规差距、数据出境和隐私合规。claude-for-legal-ZH/privacy-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 数据合规与隐私 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/privacy-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`privacy-legal`
- 领域画像与共享规则：`privacy-legal/CLAUDE.md`
- 原始技能目录：`privacy-legal/skills`
- 原插件说明：个人信息保护实务：处理活动分类、生成个人信息保护影响评估（个保法第55条）、审查个人信息处理协议（作为处理者或受托处理者）、在法定期限内起草个人信息主体权利响应（个保法第44-50条）、监测隐私政策与实践之间的偏差。
...
1. 处理实质法律工作前，先读取 `privacy-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/privacy-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/privacy-legal:some-command` 应解释为：读取 `privacy-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
