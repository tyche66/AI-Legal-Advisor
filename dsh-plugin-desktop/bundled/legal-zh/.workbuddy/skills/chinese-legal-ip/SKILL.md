...
description: "知识产权领域法律工作：商标可注册性、FTO、侵权警告函、通知删除、开源许可证、知识产权条款、专利、著作权、商标。claude-for-legal-ZH/ip-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 知识产权 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/ip-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`ip-legal`
- 领域画像与共享规则：`ip-legal/CLAUDE.md`
- 原始技能目录：`ip-legal/skills`
- 原插件说明：知识产权实务：商标可注册性检索（相同/近似分析）、自由实施（FTO）初步分析、发明披露初步专利性筛选、起草和分类侵权警告函及信息网络传播权通知（发送与应对）、开源许可证合规审查、知识产权条款审查、知识产权组合管理与续展跟踪。
...
1. 处理实质法律工作前，先读取 `ip-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/ip-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/ip-legal:some-command` 应解释为：读取 `ip-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
