...
**检查带按管辖地设置。** 始终加载 `references/plausibility-bands/全国基线.md`（《民事诉讼法》及司法解释的全国统一期限），再叠加 `references/plausibility-bands/{省份}.md`（如存在）中本诊所管辖地的地方性期限。可参照 `省份模板.md` 的结构填写本地文件。如果管辖地的省份检查带文件在冷启动时不存在，在本会话中每个条目均写入 `warnings: no-local-plausibility-band`（仍适用全国基线），并告知指导老师如何建立本地检查带文件。
