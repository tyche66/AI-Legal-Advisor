...
### 步骤0：读取、确认覆盖范围并建立合同事实账本

在“快速通读”前先解析用户文件，记录 `source_id`、文件名、解析状态、页数/章节范围和附件清单。路径不存在、内容为空、解析失败、只读取部分内容或关键附件未提供时，执行 Gate 1：停止正式审查，说明失败原因、已读取范围和补充材料选项。只有用户明确选择“临时模式”后才可继续；临时模式必须标记 `[临时模式]`，所有事实标为“基于用户粘贴内容”或 `assumed`/`待核实`。

建立唯一的合同事实账本，再进入步骤1。最小字段如下：

```json
{
  "document": {"name": "", "source_id": "", "read_status": "complete|partial|failed|empty|unavailable", "coverage": "", "attachments": []},
  "parties": [{"role": "甲方|乙方", "name": "", "source": "", "status": "confirmed|missing|ambiguous|assumed"}],
  "facts": [{"id": "", "category": "", "value": "", "source": "", "status": "confirmed|missing|ambiguous|assumed"}],
  "routing": {"contract_type": "", "our_role": "customer|vendor|unknown", "saas_review": "required|skipped|blocked", "dpa_review": "required|skipped|blocked"}
}
```

核心事实至少包括合同名称、双方、我方角色、期限、金额或金额缺失、付款机制、服务内容、数据处理、知识产权、责任上限、解除、争议解决和附件状态。`confirmed` 只能用于当前文件或用户明确输入直接支持的事实；缺失、歧义和临时假设不得写成确定事实。正文引用 DPA、价格表、SLA、服务规格、平台规则或网页政策而附件未提供时，必须写明“正文已读；附件未读”，不得声称完整合同已审查。
...
**合同说：**
> "[当前文件中的逐字或忠实短引；禁止用通用模板或“原文示例”替代]"

**事实状态：** [已确认 / 存在歧义 / 材料缺失]
**来源位置：** [账本事实 ID；条款号、页码或附件名]
...
- [ ] Gate 1：文件读取成功，阅读范围和附件状态已披露；否则仅输出阻断说明
- [ ] Gate 2：合同名称、双方、我方角色、期限、金额/缺失、付款、服务、数据、知识产权、责任上限、解除、争议解决和附件状态已入账本
- [ ] Gate 3：每个高风险或高商业摩擦问题均有真实条款号、原文短引和来源位置；覆盖率为100%
- [ ] Gate 4：报告中的实体、金额、日期、百分比、SLA、付款方式、责任上限和附件名称均能回指账本；无模板污染
- [ ] Gate 5：法律结论、谈判立场和待律师判断事项已分开
- [ ] Gate 6：交付前三问均回答“是”
- [ ] 审查指引已加载并引用
- [ ] Deal-breaker已首先检查
- [ ] 每个问题附带具体替代语言
...
任一 Gate 未通过时，不得交付完整审查备忘录；应列出失败项并请求补充或确认。
