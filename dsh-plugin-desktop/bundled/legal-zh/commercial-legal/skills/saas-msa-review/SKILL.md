...
对于以下每个类别，先从当前文件和已读取附件提取事实，再与团队立场进行对比。不要使用硬编码阈值。每一类必须输出三态结果：`原文存在`（有真实条款号/原文短引）、`原文缺失`（当前材料没有该事项）或 `附件引用未提供`（正文引用了附件、URL、平台规则或服务规格但未读取）。三态结果不得被模板改写为确定事实。

进入专项审查前，将以下字段写入共享合同事实账本：自动续约日期和周期、取消通知窗口、续约价格/调价公式、超量收费、数据导出格式与期限、终止后访问和删除、SLA 可用性/测量周期/补救、责任上限及其来源。每个字段必须记录 `source` 和 `confirmed/missing/ambiguous/assumed` 状态；原文冲突时保留两个来源并标记 `ambiguous`。
...
### 自动续约
**状态：** [原文存在 / 原文缺失 / 附件引用未提供]
**续约日期：** [日期或 missing]
**通知窗口：** [详情或 missing]
**续约价格机制：** [按原文或 missing]
**事实来源/状态：** [条款号、页码、附件；confirmed/missing/ambiguous/assumed]
**审查指引匹配：** [在立场内 / 偏离 / 未涉及]

### 价格调整
**状态：** [三态之一]
**事实来源/状态：** [条款号、原文短引；confirmed/missing/ambiguous/assumed]
[对照审查指引立场的发现]
...
### SLA
**状态：** [三态之一]
**可用性/测量/补救：** [按原文或 missing]
**事实来源/状态：** [条款号、原文短引；confirmed/missing/ambiguous/assumed]

### 再处理者
**状态：** [三态之一]
**事实来源/状态：** [条款号、原文短引或缺失附件；confirmed/missing/ambiguous/assumed]
[对照审查指引立场的发现]
