...
- Visible DSML/XML tool-call recovery is a compatibility fallback for provider responses that omit native `tool_calls`; the DeepSeek/pi-ai response-channel issue remains deferred upstream and must be tracked with raw request/response evidence.
- Attachment-backed document evidence is not yet integrated into the desktop read-integrity ledger. Dropped-file review remains dependent on the upstream attachment event projection and is deferred until that durable evidence source is exposed.
